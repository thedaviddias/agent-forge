...
metadata:
  short-description: Manage Linear issues, projects & workflows
---
...
**Codex**
1. Add the Linear MCP: `codex mcp add linear --url https://mcp.linear.app/mcp`
2. Enable remote MCP client: set `[features] rmcp_client = true` in `config.toml` or run `codex --enable rmcp_client`
3. Log in with OAuth: `codex mcp login linear`
4. After successful login, the user must restart Codex; tell them to continue with Step 1 when they return.

**Cursor**
- Add Linear to MCP config (project or `~/.cursor`). Example: `{"mcpServers": {"linear": {"command": "npx", "args": ["-y", "mcp-remote", "https://mcp.linear.app/sse", "--transport", "sse-only"]}}}` (or use the URL/transport your Cursor version expects). Complete OAuth in the browser when prompted.

**Claude Code**
- Add the Linear MCP in Claude’s MCP settings using the same endpoint (e.g. `https://mcp.linear.app/mcp` or the SSE URL). Complete OAuth when prompted.

**Windows/WSL (any tool):** If you see connection errors on Windows, try configuring the Linear MCP to run via WSL:
```json
