...
Set `IMAGE_GEN` to the path of `scripts/image_gen.py` in this skill. When the skill is installed at a known location (e.g. `~/.agents/skills/imagegen`), use that as the skill root:

```
export IMAGE_GEN="$HOME/.agents/skills/imagegen/scripts/image_gen.py"
```

Or use the path where your tool installs skills (e.g. `~/.cursor/skills/imagegen`, `~/.claude/skills/imagegen`, `~/.codex/skills/imagegen`).
