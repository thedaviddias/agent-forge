...
to avoid extra sandbox module-cache prompts (the script uses this path; other tools may use a different name).
...
Use `--app "<AppName>"` where `<AppName>` is the host app (e.g. `Codex`, `Cursor`, `Claude Code`). For agent inspection runs, keep the output in temp:
...
- App/window capture by app name (macOS only; substring match is OK; captures all matching windows). Use the host app name (e.g. `Codex`, `Cursor`, `Claude Code`):
